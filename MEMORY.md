...
- Added edudoc-owned adapter `core/exporters/hwpx_via_hwpskill.py`: Markdown -> HWPX via `skills/hwp-skill/scripts/md2hwpx.py` subprocess, validated with the skill's `validate.py`. Real HWPX tables (`hp:tbl`) + Korean text preserved, unlike the toy `hwpx_exporter`. Structured failure if the skill is absent.
- Protected-skill rule respected: the skill is called, never modified. Covered by `tests/test_hwpx_via_hwpskill.py`; full suite 18/18.
- Not yet pipeline/compose-wired (Phase 2). Closes the biggest "skill exists but not wired" gap for HWPX generation.

### 2026-07-06 - Public-plan sample CLI added

- Added `scripts/public_plan/generate_from_samples.py`.
- The CLI reads source files/directories, builds SourceProfile and DocumentPlan
  artifacts, writes `public_plan.generated.md`, and can optionally export DOCX
  with the pip-native `DocxExporter`.
- It writes `public_plan.source_profile.json`,
  `public_plan.document_plan.json`, and optional
  `public_plan.export.docx.json`.
- It does not parse reference PDFs, call an LLM, add dependencies, auto-install
  tools, or modify protected skills.
- Next natural step: public-plan validation rules for generated Markdown.

### 2026-07-06 - Public-plan Markdown generator added

- Added `core/generators/public_plan_generator.py`.
- It renders a `public_institution_plan` DocumentPlan into a conservative
  Markdown draft with sections for writing basis, background, current status,
  objectives, major tasks, schedule, budget, expected effects, future plan,
  reference samples, and missing facts.
- The generator does not call an LLM, parse PDF/HWPX files, export final
  DOCX/PDF/HWPX files, or modify protected skills.
- Missing or unsupported facts remain `확인 필요`.
- Next natural step: a small command or pipeline path for
  samples -> SourceProfile -> DocumentPlan -> public-plan Markdown.

### 2026-07-06 - SourceProfile and DocumentPlan scaffold added

- Added `core/source_profile.py` to extract deterministic source facts from
  normalized Markdown/DocumentModel input: titles, institutions, dates,
  document numbers, table summaries, statistics, budgets, schedules, key
  actions, risks, and attachments.
- Added `core/document_plan.py` to map SourceProfile facts into the
  `public_institution_plan` TargetDocumentProfile.
- The current DocumentPlan output is a planning scaffold, not a final generated
  report.
- Reference PDFs under `references/document-types/*/samples/` are tracked as
  reference sample paths but are not parsed by this layer.
- Missing or unsupported facts remain `확인 필요`.
- Next natural step: a public-plan Markdown draft generator from DocumentPlan.
...
- Profiles define required fields, source-profile facts, required sections,
  unknown-field policy, validation target, and optional HWPX renderer references.
- Profiles do not execute protected skill scripts, generate final documents,
  install dependencies, clone repositories, or modify protected skill files.
- This prepares the next SourceProfile -> TargetDocumentProfile -> DocumentPlan
  generation flow for reference-based public-document creation.
...
- Binary `.hwp` intake now prefers HWP -> temporary HWPX -> Markdown/
  DocumentModel when that safe adapter path is available.
- Existing pyhwp -> HTML -> markdownify fallback remains available when the
  adapter or intermediate HWPX read fails.
- HWP input still does not imply HWPX final output. Final rendering remains
  downstream and user/exporter controlled.

### 2026-07-03 - SourceBundle intake foundation added

- Added `core/source_bundle.py` as the first runtime layer for source/reference intake.
- SourceBundle creation builds a filtered manifest only; it does not convert, generate, validate, render, export, or write outputs.
- The builder reuses bulk input filtering rules so README/AGENTS/control files and generated artifacts do not become business source documents.
- Unsupported source candidates are recorded separately from processable documents and ignored files.
- Next implementation theme is document understanding over the SourceBundle manifest.

### 2026-07-03 - Product direction reset to document task automation

- Reframed edudoc as a document task automation system, not a file-format conversion tool.
- Canonical product flow is source/reference materials + user intent -> generated task document -> document-type validation -> final rendering.
- Source/sample files are reference material for document understanding and generation, not merely conversion input.
- Export remains important but downstream: DOCX/PDF/HWPX/PPTX are output channels, not the product goal.
- Next implementation themes: source bundle/input filtering, document understanding profile, user request planner, official report generator, document-type validators, and template/render/export after generation.

edudoc 프로젝트의 중요한 결정 기록. (CLAUDE.md의 '기억과 연속성' 규칙에 따라 관리)

## 결정 로그

### 2026-07-03 - Sample filtering and DOCX table quality baseline added

- Directory runs now skip repository/control files and generated artifacts, including `README.md`, `AGENTS.md`, validation reports, document JSON, and generated Office outputs.
- Runtime outputs remain in the configured output directory, normally `exports/`, not in `samples/`.
- DOCX form/table quality is the current Loop 8 priority before PDF rendering work.
- `DocxExporter` now applies a compact landscape strategy for wide tables and records table quality metadata.
- PDF remains fallback/experimental; HWPX remains experimental.

### 2026-07-03 - Protected skill adapter export slice completed

- Protected HWP/HWPX skill sources remain reference-only; no protected skill files were modified.
- Added edudoc-owned minimal HWPX export in `core/exporters/hwpx_exporter.py`.
- Added package-level HWPX validation in `validators/hwpx_package_rules.py`, based on safe ZIP/XML checks from protected skill references.
- Expanded pipeline export metadata with `path`, `status`, `experimental`, `requires_optional_tool`, and `note`.
- DOCX remains the partially stabilized pip-native export path.
- PDF remains fallback/experimental through `OfficeExporter`; no stable pip-native PDF exporter was added.
- HWPX export is experimental: it preserves visible text in a minimal package, but does not claim layout-perfect or institution-approved HWPX output.
