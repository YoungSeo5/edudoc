...
class HwpxTemplateInputError(RuntimeError):
    pass
...
def resolve_hwpx_template_input(
    ...
    alias_map = load_alias_map(
        ...
        if isinstance(placeholder_map.get("template_id"), str)
        else None,
    )
...
def prepare_hwpx_template_input(
    ...
    *,
    ...
    template_id = resolved.placeholder_map.get("template_id")
    package_metadata: FssPackageMetadata | None = None
    if template_id == FSS_TEMPLATE_ID:
        if requester_name is None or requested_at is None:
