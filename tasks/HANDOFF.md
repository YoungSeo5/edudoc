...
- MEMORY.md retains the historical 2026-07-01 ".hwp 격상" note; it is explicitly marked superseded by the 2026-07-02 HWPX-first decision (drift check passes).
...
- PDF: produced via fallback (~198KB/~275KB) but NOT usable for wide/complex tables (split/overlapping cells); was previously reported as plain `출력 성공`.
- Validation report: Gongmun rules report `확인 필요` (e.g. missing `끝.`) — expected for non-gongmun samples; this is a validation result, not an export failure.
...
PDF remains available via fallback but is now marked honestly. `meta["exports"]` entries carry `stabilized` (True only for the pip-native `DocxExporter`) and, for fallback formats, `experimental: True` + a layout-warning `note`. The CLI prints fallback exports as `출력(fallback·실험적)` instead of `출력 성공`. No PDF exporter was implemented.
...
No dedicated pip-native PDF exporter; `.pdf` uses the Pandoc/Typst fallback. Misleading "success" was fixed in the prior 8.97 triage (`meta["exports"]` `stabilized`/`experimental` + CLI `출력(fallback·실험적)`).
...
`docs/export-quality-criteria.md`: (1) format conversion, (2) content-preserving, (3) structure-preserving, (4) layout-aware rendering, (5) reference-based generation. Current: DOCX levels 1–2 solid + level 3 partial; PDF level 1 only (fallback); HWPX/PPTX planned; layout (4) manual-only; generation (5) only the small Gongmun harness.
