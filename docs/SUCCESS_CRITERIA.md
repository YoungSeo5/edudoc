...
- Dedicated Gongmun generation succeeds only after `gongmun_rules` validates its generated draft. `끝.` and `[붙임]` are limited to that explicit route or compose `profile_family="gongmun"`.
